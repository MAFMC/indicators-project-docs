...
\item
  ...
  addition to stock status while retaining overall simplicity of the
  approach and broad scope for consistent application across species.
  Start by reviewing cases where the Council has suspended the risk
  policy.
\item
  \textbf{Catch/quota setting:} review and communicate ecosystem/habitat
  information available/used throughout the process to ensure indicators
  are not ``double counted''. Consider information used in
  prioritization and assessment but focus on developing new indicators
  for SSC ABC, Monitoring Committee, and Council processes. Consider a
  detailed application for a particular species.
\end{itemize}
...
\item
  ...
  processes, decisions reflect reality, interannual stability, ability
  to assess tradeoffs across objectives
\item
  \textbf{Undesirable:} overly complex processes, cascading unforeseen
  impacts of decisions resulting in reactive rather than proactive
  management, accounting for conditions in more than one place (double
  counting)
\end{itemize}
